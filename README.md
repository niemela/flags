...
3. **Distinct flag artifacts from different perspectives → separate files.** When a containing state publishes — or popularly attributes to itself — a *different* flag for the same territory, both belong in the corpus as independent entries: the entity's own flag under the 3166-1 id, and the alternative attribution under the 3166-2 id. `TW` carries Taiwan's own national flag (the Blue Sky, White Sun, Red Earth); `CN-TW` carries the [flag commonly indexed under "Flag of Taiwan Province"](https://en.wikipedia.org/wiki/File:Flag_of_Taiwan_Province.svg), which is in fact the Republic of China's defunct provincial-government flag rather than anything the PRC officially publishes — the PRC's claimed Taiwan Province has no flag of its own. Use `name`, `status` (`proposed`, `de-facto`, `historical`, `alternative`), and `history` to make the actual provenance unmistakable.
...
**A complex charge is catalogued by its principal color(s), not its full internal palette.** List the colors that read at flag-flying distance — the field, the major divisions, and the *dominant tincture* of any device — and omit the incidental detail colors used only inside a coat of arms, seal, or pictorial charge (fine outlines, shading, small accents). Listing every detail color over-catalogues the palette; listing none loses the device's principal color. Worked example — `qing` (the Qing "Yellow Dragon Flag"): a yellow field bears an elaborate Azure Dragon chasing a red pearl. The dragon is rendered with many tinctures (scales, claws, eyes, flames), but its principal color is azure, so the palette is simply `yellow` (field), `blue` (dragon), `red` (pearl) — not the dozen colors the dragon's detailing actually uses. Conversely, when a device's colors *are* the flag's design — a banner of arms, a divided field, or a bold single-tincture charge — list them, because they read as the flag's own colors rather than a charge's internal detail.
...
One canonical reference per value: [pan-african](https://en.wikipedia.org/wiki/Pan-African_colours), [garvey](https://en.wikipedia.org/wiki/Pan-African_flag), [pan-arab](https://en.wikipedia.org/wiki/Pan-Arab_colors), [pan-slavic](https://en.wikipedia.org/wiki/Pan-Slavic_colors), [gran-colombian](https://en.wikipedia.org/wiki/Flag_of_Gran_Colombia), [central-american](https://en.wikipedia.org/wiki/Flag_of_Central_America), [british-ensign](https://en.wikipedia.org/wiki/Ensign_(flag)), [stars-and-stripes](https://en.wikipedia.org/wiki/Flag_of_the_United_States), [ottoman](https://en.wikipedia.org/wiki/Flags_of_the_Ottoman_Empire), [soviet](https://en.wikipedia.org/wiki/Flag_of_the_Soviet_Union), [tricolore](https://en.wikipedia.org/wiki/Flag_of_France), [prinsenvlag](https://en.wikipedia.org/wiki/Prince%27s_Flag).
